---
name: ship
description: Branch-per-turn workflow for this repo. Phase A (auto): branch from synced `main` as `claude/<type>-<slug>`, commit (conventional-commits), push, stop. Phase B (only after user confirms — "ship it" / "merge it" / "ok ship" / etc.): PR to `main`, squash-merge with `--admin --delete-branch`, return to a synced `main`. Skip for read-only turns and `.claude/plans/*`. Invoke Phase A automatically — wait only to open the PR.
---

# Branch-per-turn ship (two phases)

The only way changes land in this repo. Phase A runs automatically; Phase B waits for explicit chat confirmation. This repo only has `main` — no `dev`, no `master`, no long-lived branches.

## When to run

RUN if the turn edits any file tracked by this repo (notes, templates, configs, docs, MOCs, `.claude/`).
...
Phase A ends here. **Do not** open a PR, switch to `main`, or delete the branch. Final message must include:
...
- Prompt: *"Say `ship it` when you're ready to open a PR and squash-merge to `main`."*
...
### 10. Return to a synced `main`

```bash
git checkout main && git pull origin main && git branch -D "$BRANCH"
```

End on `main`, in sync with `origin/main`, no stale local `claude/` branches.
...
If any step (push, PR create, merge) fails: **leave state as-is** — stay on the `claude/` branch, don't delete it, don't switch to `main`. Report which step failed and the exact manual command to finish (e.g. `git push -u origin <branch>`, `gh pr create --base main --head <branch> …`, `gh pr merge <N> --squash --admin --delete-branch`).
...
- Push directly to `main`, or commit on `main` locally — always go via a `claude/…` branch + PR.
...
- **Open a PR or merge without an explicit confirmation phrase.** Pushing the `claude/` branch is fine; anything that creates visible state on `main` requires the user's go-ahead.
